...
fn main() {
    ...
    for i in (0..99999999999999_usize).rev(){
        println!("Trying {}", i);
        let digits:Vec<u32> = i.to_string()
        .chars()
    ...
    for line in reader.lines() {
        ...
        match split[0] {
            ...
            "add" => {
                ...
                result.push(alu::Instruction::Add { a: reg1, b: reg2 });
            }
            "mul" => {
                let reg1 = str_to_reg(split[1]);
                let reg2 = str_to_reg(split[2]);
                result.push(alu::Instruction::Mul { a: reg1, b: reg2 });
            }
            "div" => {
                let reg1 = str_to_reg(split[1]);
                let reg2 = str_to_reg(split[2]);
                result.push(alu::Instruction::Div { a: reg1, b: reg2 });
            }
            "mod" => {
                let reg1 = str_to_reg(split[1]);
                let reg2 = str_to_reg(split[2]);
                result.push(alu::Instruction::Mod { a: reg1, b: reg2 });
            }
            "eql" => {
                let reg1 = str_to_reg(split[1]);
                let reg2 = str_to_reg(split[2]);
                result.push(alu::Instruction::Eql { a: reg1, b: reg2 });
            }
            _ => {
